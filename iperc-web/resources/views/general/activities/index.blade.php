...
<div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header py-3 px-3">
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="btn btn-danger" data-toggle="modal" data-target="#create">Nueva Actividad</div>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="create" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('actividad.save') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <input type="hidden" id="el_id" name="el_id">
                        <label for="name">SubProceso</label>
                        <select name="sub_process" id="sub-process" class="form-control" required>
                            <option value="">Selecciona SubProceso - Zona</option>
                            @foreach ($subProcesses as $subProcess)
                                <option value="{{ $subProcess->id }}">{{ $subProcess->name }} - {{ $subProcess->zone->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
    ...
    $('body').on('click', '.table-edit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#sub-process').val(response.sub_process_id);
            },
