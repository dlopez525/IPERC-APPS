...
<div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
    <div class="statbox widget box box-shadow">
        <div class="widget-header py-3 px-3">
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="btn btn-danger" data-toggle="modal" data-target="#create">Nuevo Puesto de Trabajo</div>
                </div>
            </div>
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Nuevo Puesto de Trabajo</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                  </button>
            </div>
            <form action="{{ route('puestotrabajo.save') }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <input type="hidden" id="el_id" name="el_id">
                        <label for="name">Archivo</label>
                        <select name="iperc_file" id="iperc_file" class="form-control" required>
                            <option value="">Selecciona un Archivo</option>
                            {{-- @foreach ($subprocess as $subp)
                                <option value="{{ $subp->id }}">{{ $subp->name }}</option>
                            @endforeach --}}
                        </select>
                    </div>
                    <div class="form-group">
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.1.0-beta.1/js/i18n/es.min.js"></script>
    ...
    $('body').on('click', '.table-edit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#iperc_file').val(response.iperc_file_id);
            },
